...
const HashGenerator: React.FC<HashGeneratorProps> = ({ 
  secondarySalt = '', 
  onClear,
  visualizationMethod = 'keypad',
  initialText = '',
  initialSalt = '',
  ...
  const generateHashWithSalts = (
    textValue: string, 
    saltValue: string, 
    algorithmValue: HashAlgorithm
  ) => {
    try {
      // Combine primary salt and secondary salt
      const combinedSalt = saltValue + (secondarySalt ? `:${secondarySalt}` : '');
      const generatedHash = generateHash(textValue, algorithmValue, combinedSalt);
